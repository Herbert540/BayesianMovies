...
from os import listdir, path # A library that allows us to read every file in a directory
...
def get_total_words(freq_dict):
	...
	# You can use a for loop, or use the my_list.sum() built-in method

	list_of_words = freq_dict.values()
	return sum(list_of_words)
...
class BayesClassifier:
	...
	def train(self, directory_name): 
		...
		for file in file_names:
			if file.endswith(".txt"):
				...
				if file.startswith(self.pos_file_prefix):
					add_text_to_frequency_dict(self.pos_freqs, lines)

				elif file.startswith(self.neg_file_prefix):
					add_text_to_frequency_dict(self.neg_freqs, lines)
	...
	def save(self, filename): 
		...
		data = {"pos": self.pos_freqs, "neg": self.neg_freqs}
		json.dump(data, newfile, indent=4, sort_keys=True)
	...
	def load(self, filename): 
		...
		self.pos_freqs = reader["pos"]
		self.neg_freqs = reader["neg"]
	...
	def get_likelihood_of_word(self, word, is_positive):
		...
		Parameters: 
			...
			False if we are comparing against the negative dictionary
		Returns: 
		...
		if is_positive:
			count = self.get_pos_frequency(word) + 1
			return float(count)/get_total_words(self.pos_freqs)

		else:
			count = self.get_neg_frequency(word) + 1
			return float(count)/get_total_words(self.neg_freqs)
	...
	def get_log_likelihood_of_text(self, text, is_positive):
		...
		tokens = tokenize(text)
		for token in tokens:
			prob = self.get_likelihood_of_word(token,is_positive)
			sum_prob = sum_prob + math.log(prob)
		return sum_prob
	...
	def get_prediction(self, text):
		...
		log_likelihood_pos = self.get_log_likelihood_of_text(text,True)
		log_likelihood_neg = self.get_log_likelihood_of_text(text,False)
...
if __name__ == "__main__":
	...
	# assert math.isclose(bad_review_likelihood_pos, -27.7540326, rel_tol=1e-5, abs_tol=0.0)
	# assert math.isclose(bad_review_likelihood_neg, -18.5143893, rel_tol=1e-5, abs_tol=0.0)
	#
	# # #---------------------------------------------------------------------
	# # # Test task 9
	#
